#include <stdio.h> // Para entrada e saí­da de dados (printf, scanf, fgets) #include <stdlib.h> // Para funções do sistema (system, etc.) #include <string.h> // Para manipulação de strings (strlen, strstr) #include <ctype.h> // Para verificar caracteres (isalpha, isdigit) #include <time.h> // Para pegar a data atual do sistema

#define MAX_ESTOQUE 50 // Limite máximo de remédios que podem ser cadastrados

// Remove o caractere '\n' deixado pelo fgets no final da string void limpar_quebra_linha(char *str){ size_t len = strlen(str); if(len > 0 && str[len - 1] == '\n'){ str[len - 1] = '\0'; } }

// Limpa o buffer do teclado para evitar erros em leituras subsequentes void limpar_buffer(){ int c; while((c = getchar()) != '\n' && c != EOF); }

// Verifica se a string contém apenas letras e espaÃ§os int somente_letras(char *str){ for(int i = 0; str[i] != '\0'; i++){ if(!isalpha(str[i]) && str[i] != ' '){ // Se tiver algo que nÃ£o seja letra nem espaÃ§o return 0; // Retorna 0 (falso) } } return 1; // Retorna 1 (verdadeiro) }

// Estrutura para armazenar uma data (dia, mêss e ano) typedef struct { int dia; int mes; int ano; } data;

// Estrutura principal que representa um remédio typedef struct { char nome[100]; // Nome do remédio char forma[50]; // Forma farmacéutica (ex: comprimido, pomada) char dosagem[50]; // Dosagem (ex: 500mg) int qtdcaixa; // Quantidade de caixas em estoque data validade; // Data de validade do remédio float valor; // Valor em reais (R$) } tipo_remedio;

// Retorna a data atual do sistema data data_atual(){ time_t t = time(NULL); // Pega o tempo atual struct tm tm = *localtime(&t); // Converte para formato de data legí­vel
...
// Verifica se uma data informada é válida int data_valida(int d, int m, int a) { int dias_mes[] = {0,31,28,31,30,31,30,31,31,30,31,30,31};
...
int main(){ tipo_remedio estoque[MAX_ESTOQUE]; // Vetor de remédios data hoje = data_atual(); // Pega a data atual int i, escolha, escolha1, escolha2; int qtdremedios, cadastro = 0, senha, parou = 0, qtdestoque, validos; int mes_diff;
